    ...
    def get(self, src_id):
		...
		#delete training folder if exists to avoid conflicts with previous training set
                ...
                if config['MODEL_TYPE']=='caffe':
                    trainer_def={"model-repo":os.path.abspath(model_path),"training-repo":os.path.abspath(training_path),"sname":model["_source"]["title"]+"_trainer","tsplit":0.2,"base-lr":0.05,"clevel":False,"sequence":140,"iterations":2000,"test-interval":200,"stepsize":15000,"destroy":True,"resume":True,"finetune":False,"weights":None,"nclasses":len(model["_source"]["related_topics"]),"documents":True,"batch_size":300,"test_batch_size":16,"gpu":config['MODEL_GPU'],"gpuid":0,"mllib":"caffe","lregression":False}
                else:
