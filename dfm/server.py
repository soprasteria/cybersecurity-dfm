    ...
    for doc in docs['hits'][0:3]:
        ...
        if isinstance(doc, list):
            ...
            try:
               ...
               results.add_success({'object':json.dumps(list(doc)),'message':'added to processing queue','queue_size':work_queue.qsize()})
            except Exception as e:
                app.logger.exception("can't queue: "+str(list(doc)))
                results.add_fail({'object':json.dumps(list(doc)),'message':'fail to add to processing queue','queue_size':work_queue.qsize()})
        sys.stdout.flush()
